    ...
    *  Otherwise, creates a new user with the given name, adds the user to this network, and returns true. */
    ...
    public String recommendWhoToFollow(String name) {
        ...
        for(int i=0; i<userCount; i++){
            if(users[i] != null && users[i] == user1){
                continue;
            }
            int currentMutal = user1.countMutual(users[i]);
            if(currentMutal > maxMutal){
                maxMutal = currentMutal;
    ...
    public String mostPopularUser() {
        ...
        for(int i=0; i < userCount; i++){
            ...
            int check = followeeCount(currentUser);
            if(check > countPopular){
                countPopular = check;
